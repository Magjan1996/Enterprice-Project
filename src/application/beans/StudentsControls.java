...
public class StudentsControls {
    ...
    public class AddStudentController{
        ...
        public void addStudent(){
            if(name.length()==0){
                FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR, "Заполните поля", "Введите имя студента"));
            }else{
                if(surname.length()==0){
                    FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR, "Заполните поля", "Введите фамилию студента"));
                }else{
                    if(faculty.length()==0){
                        FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR, "Заполните поля", "Введите факультет студента"));
                    }else{
                        add(name,surname,fathername,faculty);
                    }
                }
            }
        ...
        private void add(String name, String surname, String fathername, String faculty) {
